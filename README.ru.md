...
Код разделён на общую часть (`common.py`) и вариантные файлы (`app.py` для WG/AWG, `app_xray.py` для Xray). Никаких `if WG_VARIANT` — полное разделение.

## Возможности

- управление клиентами (создать/удалить/переименовать)
- QR-коды и конфиги (WG, AWG, Xray VLESS-ссылки для REALITY/XHTTP/WS)
- роли (admin / user)
- статистика трафика (WG/AWG)
- журнал действий
- резервные копии / восстановление
- ограничение скорости (WG/AWG)
- родительский контроль (WG/AWG)
